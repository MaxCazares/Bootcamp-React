...
import { SkipForward, Play, Newspaper } from 'lucide-react';
...
export const ScrambleWords = () => {
	...
	const handleSkip = () => {
		...
		const skippedWords = words.slice(1);
		setWords(skippedWords);
		setCurrentWord(skippedWords[0]);
		setScrambledWord(scrambleWord(skippedWords[0]));
		setGuess('');
	};
	...
	const handlePlayAgain = () => {
		...
		const newGame = shuffleArray(GAME_WORDS);
		setWords(newGame);
		setCurrentWord(newGame[0]);
		setScrambledWord(scrambleWord(newGame[0]));
	};
